...
export class NameChange extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save NameChange entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type NameChange must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("NameChange", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): NameChange | null {
    return changetype<NameChange | null>(
      store.get_in_block("NameChange", id.toHexString())
    );
  }

  static load(id: Bytes): NameChange | null {
    return changetype<NameChange | null>(
      store.get("NameChange", id.toHexString())
    );
  }
  ...
  get maskIndex(): BigInt {
    let value = this.get("maskIndex");
    if (!value || value.kind == ValueKind.NULL) {
      throw new Error("Cannot return null for a required field.");
    } else {
      return value.toBigInt();
  ...
  set maskIndex(value: BigInt) {
    this.set("maskIndex", Value.fromBigInt(value));
  }
